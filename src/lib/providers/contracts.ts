...
export interface DeploymentProvider { getProject(): Promise<{ id: string; name: string } | null>; createProject(name: string): Promise<{ id: string; name: string }>; setEnvironmentVariable(name: string, value: string): Promise<void>; deploy(files: Array<{ path: string; content: string }>): Promise<{ id: string; url: string }>; getDeploymentStatus(id: string): Promise<"queued" | "building" | "ready" | "error">; getLogs(id: string): Promise<string[]>; }
